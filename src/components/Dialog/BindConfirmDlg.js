...
export default function BindConfirmDlg({ open, onClose, onClick, disabled, activeNodes = [] }) {
  ...
  return (
    <Dialog
      ...
      aria-describedby="bind-dialog-description"
      PaperProps={{
        sx: {
          background: '#161C24',
          borderRadius: '40px',
    ...
    >
      <DialogTitle id="bind-dialog-title">
        <HeaderTypo sx={{ py: 1, textAlign: 'center' }}>{t('btn-bind')}</HeaderTypo>
      </DialogTitle>
      <DialogContent>
        <DialogContentText
          id="bind-dialog-description"
          sx={{ display: 'flex', justifyContent: 'center' }}
        >
          <NormalTypo component="span" sx={{ py: 1, px: { xs: 1, md: 2 }, textAlign: 'center' }}>
            {t('dlg-bind-confirm-label-1')} {t('dlg-bind-confirm-label-2')}
          </NormalTypo>
        </DialogContentText>
        <Box
          component="img"
          src="/static/ic_bind.svg"
          alt="bind_icon"
          width="60px"
        ...
        <Stack spacing={0.5} px={{ xs: 0, md: 2 }}>
          ...
          {error > 0 && (
            <Typography fontSize={12} fontWeight={500} color="#EB5757">
              {t('dlg-bind-confirm-node-select-label')}
            </Typography>
          )}
        </Stack>
        <Stack
