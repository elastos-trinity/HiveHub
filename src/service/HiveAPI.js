...
export const registerScripting = (scriptName) =>
  new Promise((resolve, reject) => {
    const executablefilter = { property_identity: scriptName, type: 'public' };
    const options = { projection: { _id: false }, limit: 1 };
    const executable = new FindExecutable(
      'find_message',
      COLLECTION_NAME,
      executablefilter,
      options
...
export const queryName = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(NAME, targetDid, appid);
  return result;
};

export const queryDescription = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(DESCRIPTION, targetDid, appid);
  return result;
};

export const queryWebsite = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(WEBSITE, targetDid, appid);
  return result;
};

export const queryTwitter = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(TWITTER, targetDid, appid);
  return result;
};

export const queryDiscord = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(DISCORD, targetDid, appid);
  return result;
};

export const queryTelegram = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(TELEGRAM, targetDid, appid);
  return result;
};

export const queryMedium = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(MEDIUM, targetDid, appid);
  return result;
};

export const queryKycMe = async (targetDid, appid = config.ApplicationDID) => {
  const result = await callQueryScript(KYC_ME, targetDid, appid);
  return result;
};

// const callQueryScript = async(propertyIdentity, targetDid, appid = config.ApplicationDID) => {
//   try {
...
const callQueryScript = (propertyIdentity, targetDid, appid = config.ApplicationDID) =>
  new Promise((resolve, reject) => {
    const doc = { property_identity: propertyIdentity };
